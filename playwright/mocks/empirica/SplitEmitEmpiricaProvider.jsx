 ...
 */
export const SplitEmitEmpiricaProvider = React.forwardRef(
  function SplitEmitEmpiricaProvider(
    {
      currentPlayerId,
    ...
    const ctxRef = useRef({
      ...
      ctxGame: initialGame
        ? initialGame instanceof MockGame
          ? initialGame
          : new MockGame(initialGame.attrs ?? {})
        : null,
    ...
    const [emittedStage, setEmittedStage] = useState(
      () => ctxRef.current.ctxStage,
    );
    const [emittedGame, setEmittedGame] = useState(
      () => ctxRef.current.ctxGame,
    );
    const [emittedRound, setEmittedRound] = useState(
      () => ctxRef.current.ctxRound,
    );
    ...
    useLayoutEffect(() => {
      ...
      for (const playerId of initialPlayerIds) {
        ...
        ctxRef.current.playerStageMap.set(
          `${playerId}:${stageId}`,
          playerStage,
        );
      }
