...
async function advanceToStep3(page) {
  ...
  await page.locator('[data-test="speakerSelect"]').selectOption('speaker-1');
...
test('HC-001: headphones button reveals speaker selection', async ({ mount, page }) => {
  ...
  // Step 2 now visible
  await expect(page.locator('[data-test="speakerSelect"]')).toBeVisible();
...
test('HC-002: speaker selection advances to Step 3', async ({ mount, page }) => {
  await installAudioMocks(page);
  ...
  // Select speaker
  await page.locator('[data-test="speakerSelect"]').selectOption('speaker-2');
...
test('HC-004: wrong sound identification does not pass', async ({ mount, page }) => {
  await installAudioMocks(page);
  ...
  // Select wrong answer
  await page.locator('[data-test="soundSelect"] input[value="dog"]').check();
...
test('HC-006: change speaker resets to selection', async ({ mount, page }) => {
  ...
  await expect(page.locator('[data-test="speakerSelect"]')).toBeVisible();
  await expect(page.locator('[data-test="playSound"]')).not.toBeVisible();
});
...
test('HC-008: setSinkId routes audio to selected speaker', async ({ mount, page }) => {
  await installAudioMocks(page);
