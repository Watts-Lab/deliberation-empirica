...
test.describe("Debrief Page", () => {
  test("DEBRIEF-001: custom debrief markdown renders from CDN", async ({
    mount,
    page,
  }) => {
    ...
    await expect(
      component.getByText("This was a test of group deliberation."),
    ).toBeVisible();
    // Generic message should not appear when custom content is loaded
    await expect(
      component.getByText("Thank you for participating.", { exact: true }),
    ).not.toBeVisible();
  });

  test('DEBRIEF-002: generic message when debrief is "none"', async ({
    mount,
    page,
  }) => {
  ...
  test("DEBRIEF-003: exit code displayed when provided", async ({
    mount,
    page,
  }) => {
  ...
  test('DEBRIEF-004: exit code section hidden when exitCodes is "none"', async ({
    mount,
    page,
  }) => {
    await setupGlobals(page);
  ...
  test("DEBRIEF-005: copy to clipboard button triggers alert with code", async ({
    mount,
    page,
  }) => {
    await setupGlobals(page);
  ...
  test("DEBRIEF-006: loading state when player is null", async ({
    mount,
    page,
  }) => {
    await setupGlobals(page);
  ...
  test("DEBRIEF-007: inline loading while custom debrief is being fetched", async ({
    mount,
    page,
  }) => {
  ...
  test('DEBRIEF-009a: "You may now close this window" visible with exit codes', async ({
    mount,
    page,
  }) => {
    await setupGlobals(page);
    ...
    await expect(
      component.getByText("You may now close this window."),
    ).toBeVisible();
  });

  test('DEBRIEF-009b: "You may now close this window" visible without exit codes', async ({
    mount,
    page,
  }) => {
    await setupGlobals(page);
