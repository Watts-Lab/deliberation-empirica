 ...
 * browser-version gate in the same component. See issue #22.
 *
...
async function setMockEnv(page, { isMobile = false, browser } = {}) {
  await page.evaluate(
    (env) => {
      ...
      // Use undefined as the "no override" signal so tests can pass
      // `browser: null` to exercise the `detect()` returns null branch.
      window.__mockBrowser =
        env.browser === undefined
          ? { name: "chrome", version: "120.0.0", os: "mac" }
          : env.browser;
