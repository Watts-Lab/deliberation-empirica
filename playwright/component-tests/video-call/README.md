...
_Different Player Counts (5 tests):_
...
_Different Screen Widths (3 tests):_
...
_Layout Quality Checks (3 tests):_
...
_Layout Adapts to Container Size (4 tests):_
...
```javascript
test("Safari audio context recovery with custom speaker", async ({
  mount,
  page,
}) => {
  ...
  // Join call
  await page.waitForSelector('[data-testid="callTile"]');
