...
test.describe("Daily Hooks Behavior", () => {
  ...
  test("investigate hook behavior over time", async ({ mount, page }) => {
    ...
    const apiHasSessionId = snapshots.some(
      (s) =>
        s.localParticipant?.session_id !== null &&
        s.localParticipant?.session_id !== undefined,
    );
    ...
    if (!hookEverReturned) {
      ...
      console.log(
        "   Even though Daily API has session_id:",
        sessionIdTimeline[sessionIdTimeline.length - 1]?.apiValue,
      );
      console.log(
    ...
    console.log(
      JSON.stringify(
        {
          "useLocalSessionId() returned":
            finalSnapshot.localSessionId || "null",
          "Daily API session_id":
            finalSnapshot.localParticipant?.session_id || "null",
          "useVideoTrack() state": finalSnapshot.videoTrack?.state || "null",
          "Daily API video state":
            finalSnapshot.localParticipant?.videoState || "null",
          "useAudioTrack() state": finalSnapshot.audioTrack?.state || "null",
          "Daily API audio state":
            finalSnapshot.localParticipant?.audioState || "null",
          "useParticipantProperty() userName": finalSnapshot.userName || "null",
          "Daily API user_name":
            finalSnapshot.localParticipant?.user_name || "null",
        },
        null,
        2,
      ),
  ...
  test("investigate hook behavior with explicit session ID", async ({
    mount,
    page,
  }) => {
    await page.context().grantPermissions(["camera", "microphone"]);
    ...
    if (otherHooksWork) {
      ...
      console.log(
        "   → This suggests useLocalSessionId is the only broken hook",
      );
    } else {
      console.log(
      ...
      console.log(
        "   → This suggests a broader issue with Daily React hooks in Playwright CT",
      );
    }
  });
});
