...
test.describe("MockEmpiricaProvider Reactivity Debug", () => {
  test("TEST 1: Player reader sees initial attributes", async ({
    mount,
    page,
  }) => {
    const config = {
      empirica: {
        currentPlayerId: "p0",
        players: [{ id: "p0", attrs: { position: "0", name: "Test Player" } }],
        game: { attrs: {} },
        stage: { attrs: {} },
      },
    };
    ...
    await expect(page.locator('[data-testid="playerId"]')).toHaveText("p0");
    await expect(page.locator('[data-testid="position"]')).toHaveText("0");
    await expect(page.locator('[data-testid="dailyId"]')).toHaveText("null");
  ...
  test("TEST 2: Player reader updates when player.set() is called from browser", async ({
    mount,
    page,
  }) => {
    const config = {
      empirica: {
        currentPlayerId: "p0",
        players: [{ id: "p0", attrs: { position: "0" } }],
        game: { attrs: {} },
        stage: { attrs: {} },
      },
    };
    ...
    const domDailyId = await page
      .locator('[data-testid="dailyId"]')
      .textContent();
  ...
  test("TEST 3: Player reader updates when sibling component calls player.set()", async ({
    mount,
    page,
  }) => {
    const config = {
  ...
  test("TEST 4: Direct player instance check - verify set() mutates object", async ({
    mount,
    page,
  }) => {
    const config = {
  ...
  test("TEST 5: Monitor forceUpdate calls - does onChange trigger re-renders?", async ({
    mount,
    page,
  }) => {
    const config = {
  ...
  test("TEST 6: Track player object identity across re-renders", async ({
    mount,
    page,
  }) => {
    ...
    page.on("console", (msg) => {
      const text = msg.text();
      if (
        text.includes("[MockEmpiricaProvider]") ||
        text.includes("[PlayerIdentityTracker]")
      ) {
