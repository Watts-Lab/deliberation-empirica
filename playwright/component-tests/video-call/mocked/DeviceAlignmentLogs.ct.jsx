...
test.describe("Device Alignment Log Behavior", () => {
   ...
   */
  test("LOG-002: no alignment log when device list is empty", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    const deviceChangedLogs = console.matching(
      /Logged stage event: device-changed/,
    );
   ...
   */
  test("LOG-001: alignment log fires once when device matched", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    const config = {
      ...
      daily: {
        ...
        devices: {
          cameras: [
            { device: { deviceId: "cam-1", label: "FaceTime Camera" } },
          ],
          microphones: [
    ...
    const alignmentLogs = console.matching(
      /Logged stage event: device-changed/,
    );
   ...
   */
  test("LOG-003: fallback Sentry message includes available device list", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    const config = {
      ...
      daily: {
        ...
        devices: {
          ...
          cameras: [
            { device: { deviceId: "cam-fallback", label: "Fallback Webcam" } },
          ],
          microphones: [
            { device: { deviceId: "mic-1", label: "Built-in Mic" } },
          ],
    ...
    // Should have a "not found, auto-switching to fallback" Sentry message
    ...
    expect(
      fallbackMsg.hint.extra.availableDevices.length,
    ).toBeGreaterThanOrEqual(1);

    // Should include the ID of the available device
    const availableIds = fallbackMsg.hint.extra.availableDevices.map(
      (d) => d.id,
    );
