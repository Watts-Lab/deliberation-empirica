 ...
 */
test('DEVICE-007: skips alignment when preferred device is "waiting"', async ({
  mount,
}) => {
 ...
 */
test("DEVICE-004: component renders when device preferences are set", async ({
  mount,
}) => {
  test.slow();
  const config = {
    ...
    daily: {
      ...
      devices: {
        ...
        microphones: [
          { device: { deviceId: "mic-1", label: "Blue Yeti USB" } },
        ],
        speakers: [
          { device: { deviceId: "spk-1", label: "Built-in Speaker" } },
        ],
 ...
 */
test("DEVICE-008 (component): renders when current device matches preference", async ({
  mount,
}) => {
  test.slow();
  const config = {
