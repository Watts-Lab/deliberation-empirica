...
test.describe("W5: Fatal error recovery", () => {
   ...
   */
  test("WF5-001: connection-error shows Connection lost message", async ({
    mount,
    page,
  }) => {
    ...
    await expect(page.locator('[data-testid="rejoinCall"]')).toBeVisible();
  });
   ...
   */
  test("WF5-002: ejected shows removed message without rejoin", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
    ...
    await expect(page.locator('[data-testid="rejoinCall"]')).not.toBeVisible();
  });
   ...
   */
  test("WF5-004: rejoin button triggers callObject.join()", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
    ...
    await page.locator('[data-testid="rejoinCall"]').click();
...
test.describe("W6: Network interruption banner", () => {
   ...
   */
  test("WF6-001: network interrupted shows reconnecting banner", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   */
  test("WF6-002: brief interruption does not show banner", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   */
  test("WF6-002b: network reconnected clears banner", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   */
  test("WF6-003: network interruption sends Sentry breadcrumb", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
...
test.describe("Sentry on Fix A/V submission", () => {
   ...
   */
  test("WF-SENTRY-001: Fix A/V submission sends Sentry with reported issues", async ({
    mount,
    page,
  }) => {
    test.slow();
...
test.describe("W1: Permission revocation proactive UI", () => {
   ...
   */
  test("WF1-MID-001: camera permission revoked shows guidance", async ({
    mount,
    page,
  }) => {
    ...
    await page.waitForFunction(() => !!window.mockCamPerm?._onchange, {
      timeout: 10000,
    });
   ...
   */
  test("WF1-MID-002: permission re-granted triggers auto-reload", async ({
    mount,
    page,
  }) => {
    await installPermissionsMock(page, "granted", "granted");
...
test.describe("W4: Device reconnected auto-recovery", () => {
   ...
   */
  test("WF4-001: devicechange during camera error auto-recovers", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   */
  test("WF4-002: devicechange during mic error auto-recovers", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   */
  test("WF4-003: devicechange ignored for non-not-found errors", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
...
test.describe("W2: Device fallback banner on not-found errors", () => {
   ...
   */
  test("WF2-001: single alternative camera shows banner", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   * a non-modal banner appears (not a picker/modal). Call tiles remain visible.
   */
  test("WF2-002: single alternative mic shows banner", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
...
test.describe("W7: Proactive track monitoring", () => {
   ...
   */
  test("WF7-001: ended audio track triggers auto-recovery", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   */
  test("WF7-002: ended video track triggers auto-recovery", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
   ...
   */
  test("WF7-003: track auto-recovery logs Sentry breadcrumb", async ({
    mount,
    page,
  }) => {
    const component = await mount(<VideoCall showSelfView />, {
...
test.describe("Device error render storm prevention", () => {
   ...
   */
  test("WF-STORM-001: rapid duplicate mic-error events do not cause render storm", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    await page.evaluate(() => {
      for (let i = 0; i < 50; i++) {
        window.mockCallObject.emit("mic-error", {
          ...
          errorMsg: {
            type: "not-found",
            message: "Preferred microphone not found",
          },
        });
      }
    });
    ...
    const depthErrors = consoleCapture
      .getErrors()
   ...
   */
  test("WF-STORM-003: rapid alignment fallback calls do not cause render storm", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    await page.evaluate(() => {
      for (let i = 0; i < 50; i++) {
        window.mockCallObject.emit("available-devices-updated", {
          ...
          microphone: [
            { deviceId: "builtin-mic-id", label: "MacBook Pro Microphone" },
          ],
        });
      }
    });
   ...
   */
  test("WF-STORM-002: rapid duplicate camera-error events do not cause render storm", async ({
    mount,
    page,
  }) => {
    test.slow();
