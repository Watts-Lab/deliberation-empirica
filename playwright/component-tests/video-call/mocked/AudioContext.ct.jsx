...
test.describe("AudioContext banner behavior", () => {
   ...
   */
  test("AUDIO-001: detects suspended AudioContext state", async ({
    mount,
    page,
  }) => {
    ...
    await expect(component.locator("text=Audio is paused")).toBeVisible({
      timeout: 10000,
    });
  });
   ...
   */
  test("AUDIO-002: shows enable audio banner when suspended", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    await expect(
      page.locator("text=Audio is paused. Click below to enable sound."),
   ...
   */
  test("AUDIO-003: clicking enable audio hides the banner", async ({
    mount,
    page,
  }) => {
    test.slow();
   ...
   */
  test("AUDIO-005: no banner when AudioContext is running", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    await expect(page.locator("text=Audio is paused")).not.toBeVisible();
    await expect(
      page.locator('button:has-text("Enable audio")'),
    ).not.toBeVisible();
  });
   ...
   */
  test("AUDIO-004: auto-resume attempts when suspended and gesture detected", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    await page.evaluate(() => {
      window.mockAudioCtx.resumeCallCount = 0;
    });
    ...
    const resumeCount = await page.evaluate(
      () => window.mockAudioCtx.resumeCallCount,
    );
   ...
   */
  test("AUDIO-006: logs AudioContext state changes", async ({
    mount,
    page,
  }) => {
    test.slow();
    ...
    const creationLogs = consoleCapture.matching(
    ...
    const stateChangeLogs = consoleCapture.matching(
      /\[Audio\] AudioContext state changed/,
    );
