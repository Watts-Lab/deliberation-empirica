...
test('FIXAV-020: device pickers visible when modal opens', async ({ mount }) => {
  const component = await mount(
    <Tray {...trayProps} />,
    { hooksConfig: configWithMultipleDevices }
  );
  await component.locator('[data-test="fixAV"]').click();
  await expect(component.locator('[data-test="deviceManagement"]')).toBeVisible();
...
test('FIXAV-021: camera picker lists available cameras', async ({ mount }) => {
  ...
  await component.locator('[data-test="fixAV"]').click();
  const cameraSelect = component.locator('[data-test="fixAVCameraSelect"]');
  await expect(cameraSelect).toBeVisible();
  await expect(cameraSelect.locator('option')).toHaveCount(2);
  await expect(cameraSelect.locator('option').first()).toHaveText('Built-in Camera');
...
test('FIXAV-023: mic picker lists available microphones', async ({ mount }) => {
  ...
  await component.locator('[data-test="fixAV"]').click();
  const micSelect = component.locator('[data-test="fixAVMicSelect"]');
...
test('FIXAV-024: speaker picker lists available speakers', async ({ mount }) => {
  const component = await mount(
    <Tray {...trayProps} />,
    { hooksConfig: configWithMultipleDevices }
  );
  await component.locator('[data-test="fixAV"]').click();
  const speakerSelect = component.locator('[data-test="fixAVSpeakerSelect"]');
...
test('FIXAV-025: test sound button exists for speaker', async ({ mount }) => {
  ...
  await component.locator('[data-test="fixAV"]').click();
  await expect(component.locator('[data-test="fixAVTestSound"]')).toBeVisible();
});

/** FIXAV-026: Diagnostics section is collapsed by default */
test('FIXAV-026: diagnostics section collapsed by default', async ({ mount }) => {
  const component = await mount(
    <Tray {...trayProps} />,
    { hooksConfig: configWithMultipleDevices }
  );
  ...
  await expect(component.locator('[data-test="diagnosticsSection"]')).not.toBeVisible();
  // But the "Having trouble?" link should be
  await expect(component.locator('[data-test="expandDiagnostics"]')).toBeVisible();
});
...
test('FIXAV-027: diagnostics section expands on click', async ({ mount }) => {
  ...
  await component.locator('[data-test="fixAV"]').click();
  await component.locator('[data-test="expandDiagnostics"]').click();
  await expect(component.locator('[data-test="diagnosticsSection"]')).toBeVisible();
...
test('FIXAV-028: self-view preview visible in modal', async ({ mount }) => {
  ...
  await component.locator('[data-test="fixAV"]').click();
  await expect(component.locator('[data-test="fixAVSelfView"]')).toBeVisible();
});
...
test('FIXAV-029: mic level indicator visible in modal', async ({ mount }) => {
  ...
  await component.locator('[data-test="fixAV"]').click();
  await expect(component.locator('[data-test="fixAVMicLevel"]')).toBeVisible();
});
