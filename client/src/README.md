...
- `Stage.jsx` — Wraps stagebook's `<Stage>` in the platform's adapter (`components/stagebookAdapter/`) + `StageProgressLabelProvider`.
...
- `components/` — Platform UI, adapters, and stagebook render-slot components. Contains `stagebookAdapter/` (Provider + pure helpers), the `discussion/` subtree (video + text chat), `Survey.jsx`, `SharedNotepad.jsx`, `stageCoherence/` (stage-transition gate + recovery), and platform primitives (`IdleProvider`, `Alert`, `Modal`, `Toast`, `Select`, `ConfirmLeave`, `ConditionalRender`, `EmpiricaMenu`, `PermissionRecovery`, `Markdown` wrapper, `ProfileTimer`, `progressLabel/`).
...
The platform supplies the `StagebookContext` via `components/stagebookAdapter/Provider.jsx`, which translates Empirica's hook-based state (`usePlayer`, `useGame`, `usePlayers`, `useGlobal`) into stagebook's `get(key, scope)` / `save(key, value, scope)` / `getAssetURL(path)` / `getTextContent(path)` contract. Pure logic lives in `components/stagebookAdapter/helpers.js` and is unit-tested in `helpers.test.js`.
