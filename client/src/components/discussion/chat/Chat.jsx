...
export function Chat({
  ...
  const actionsLength = scope?.getAttribute(attribute)?.items?.length;
  useEffect(() => {
    const actions = scope?.getAttribute(attribute)?.items || [];
    const reconstructed = reconstructChatState(actions);
    setMessages(reconstructed);
  }, [actionsLength, scope, attribute]); // Depend on items length
  ...
  const handleSendMessage = (text) => {
    ...
    scope.append(attribute, {
      ...
      sender: {
        id: player?.id,
        name: player?.get("name") || `Player ${currentPlayerPosition}`,
