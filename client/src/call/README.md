...
- `VideoCall.jsx` — Main container used by stages with `discussion.chatType: "video"`. Responsibilities:
  ...
  - Emits `callStarted` to the stage and starts recording client-side via `callObject.startRecording()` (issue #949).
...
1. `VideoCall` mounts, joins the Daily room, signals `callStarted`, and starts recording client-side if enabled.
