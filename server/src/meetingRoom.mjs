...
export async function GetRoom(playerName, roomName) {
  try {
    ...
    if (err.response) {
      console.log(`Request for access key to room ${roomName}} failed with status ${err.response.status}`);
      console.log(err.response.data);
    } else {
      console.log(`Error occured while requesting access key to room ${roomName}`);
