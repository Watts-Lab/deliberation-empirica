...
Empirica.onChange("player", "playerComplete", function ({ isNew, player }) {
  if (player.get("playerComplete") && !player.get("dollarsOwed")) {
    ...
    if (dollarsOwed > config.highPayAlert) {
      console.warn(
        "High payment for " + player.participant.identifier + ": " + dollarsOwed
      );
    }
    console.log(
      "Owe " +
...
Empirica.onNewBatch(async function ({ batch }) {
  ...
  pluckUniqueFactors(treatments, "topic").forEach(async (topicFile) => {
    const url =
      "https://raw.githubusercontent.com/Watts-Lab/deliberation-topics/main/topics/" +
      topicFile;
    let topic;
    try {
  ...
  pluckUniqueFactors(treatments, "QCSurvey").forEach(async (surveyFile) => {
    const url =
      "https://raw.githubusercontent.com/Watts-Lab/surveys/main/src/surveys/" +
      surveyFile;
    let survey;
...
Empirica.onChange("player", "roomName", async function ({ isNew, player }) {
  if (player.get("roomName") && !player.get("accessKey")) {
    ...
    console.log(
      `Set access key for player ${
        player.participant.identifier
      } in room ${player.get("roomName")} with room id ${id} to ${player.get(
        "accessKey"
      )}`
    );
    ...
    console.log(
      `Player ${player.participant.identifier} leaving room. Setting access key to null.`
    );
  }
});
