...
- **Qualtrics elements** require `QUALTRICS_API_TOKEN` and `QUALTRICS_DATACENTER` env vars; validation will throw if missing. At runtime, Deliberation Lab appends `deliberationId` and `sampleId` as URL params; any `params` entries are also appended (with `reference` values resolved from player/game state); submitted Qualtrics responses are fetched (if API keys) and stored under `qualtrics_<step>` in science data.
