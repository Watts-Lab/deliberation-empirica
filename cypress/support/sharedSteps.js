...
const empiricaMenuSelectors = (playerKey) => ({
  toggle: `[data-testid="empiricaMenuToggle"][data-player-id="${playerKey}"]`,
  panel: `[data-testid="hiddenMenu"][data-player-id="${playerKey}"]`,
});
...
Cypress.Commands.add("stepIntro", (playerKey) => {
  ...
  cy.get(`[data-player-id="${playerKey}"] [data-testid="inputPaymentId"]`)
    .should("be.visible")
    .should("be.enabled")
  ...
  cy.get(`[data-player-id="${playerKey}"] [data-testid="joinButton"]`).click();
...
Cypress.Commands.add("submitStage", (playerKey) => {
  ...
  cy.get(panel).find(`[data-testid="devSubmitStage"]`).click({ force: true });
...
Cypress.Commands.add(
  ...
  (
    ...
    if (setupMicrophone || setupHeadphones) {
      ...
      const audioButton = `[data-player-id="${playerKey}"] button[data-testid="startAudioSetup"]`;
...
Cypress.Commands.add("waitForGameLoad", (playerKey) => {
  cy.log(`⌛️ Wait: lobby`);
  cy.get(`[data-player-id="${playerKey}"]`).find('[data-testid="profile"]', {
    timeout: 20000,
...
Cypress.Commands.add("stepPreQuestion", (playerKey) => {
  ...
  cy.get(
    `[data-player-id="${playerKey}"] [data-testid="element-prompt-projects/example/multipleChoice.md"] input[value="1"]`
  ).click();

  cy.get(
    `[data-player-id="${playerKey}"] [data-testid="element-prompt-projects/example/multipleChoiceWizards.md"] input[value="6"]`
  ).click();

  cy.get(
    `[data-player-id="${playerKey}"] textarea[data-testid="element-prompt-projects/example/openResponse.md"]`
  ...
  cy.get(
    `[data-player-id="${playerKey}"] [data-testid="element-prompt-projects/example/multipleChoiceColors.md"]`
  ).should("not.exist");
