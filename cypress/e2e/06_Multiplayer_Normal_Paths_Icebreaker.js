...
describe(
  ...
  () => {
    ...
    it("walks properly", () => {
      ...
      //Consent and Login
      cy.empiricaLoginMultiPlayers(playerKeys).then(() => {
        start = dayjs();
        cy.log(`start: ${start}`);
      });
      ...
      cy.get("[test-player-id='player1']").contains("In this study", {
        timeout: 5000,
      });
      cy.get("[test-player-id='player1']").contains(
      ...
      cy.get("[test-player-id='player1']")
        ...
        .click();
      cy.get("[test-player-id='player1']").find("label").contains("No").click();
      cy.get("[test-player-id='player1']")
        .find("label")
      ...
      cy.log("Intro: instructions and understanding check");
      cy.get("[test-player-id='player2']").contains("In this study", {
        timeout: 5000,
      });
      ...
      // Video check1
      cy.get("[test-player-id='player1']").contains("Check your webcam", {
        timeout: 5000,
      });
      ...
      // Video check2
      cy.get("[test-player-id='player2']").contains("Check your webcam", {
        timeout: 5000,
      });
      //cy.get('[data-test="enableIframe"]').uncheck({force: true}) // default disabled in cypress
      ...
      cy.get("[test-player-id='player1']").find('[data-test="profile"]', {
        timeout: 20000,
      ...
      cy.get("[test-player-id='player2']").find('[data-test="profile"]', {
        timeout: 20000,
      }); // check that profile loaded
      ...
      cy.get("[test-player-id='player1']").contains("as a group", {
        timeout: 15000,
      });
      cy.get("[test-player-id='player1']")
        .get('input[value="Neither agree nor disagree"]')
      ...
      cy.get("[test-player-id='player2']").contains(
        `${playerKey1 + "_name"} changed the selected answer`
      );
      cy.get("[test-player-id='player2']")
        .get('input[value="Neither agree nor disagree"]')
      ...
      cy.get("[test-player-id='player1']").contains(
        `${playerKey2 + "_name"} changed the selected answer`
      ...
      cy.get("[test-player-id='player1']").contains("Quality Feedback Survey", {
        timeout: 5000,
      });
      ...
      cy.get(
        ...
        .click()
        .type(`Check_${playerKey1}_text_entry`);
      ...
      cy.get("[test-player-id='player2']").contains("Quality Feedback Survey", {
        timeout: 5000,
      });
      cy.wait(1000); // flake mitigation
      cy.get("[test-player-id='player2']")
      ...
      cy.get(
        ...
        .click()
        .type(`Check_${playerKey2}_text_entry`);
