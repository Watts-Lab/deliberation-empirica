...
describe(
  ...
  () => {
    ...
    it('walks properly', () => {
      ...
      cy.get(`[test-player-id="${playerKeys[0]}"]`).contains('In this study', { timeout: 5000 });
      ...
      cy.get(`[test-player-id="${playerKeys[0]}"]`)
        ...
        .click();
      cy.get(`[test-player-id="${playerKeys[0]}"]`).find('label').contains('No').click();
      cy.get(`[test-player-id="${playerKeys[0]}"]`)
        .find('label')
      ...
      cy.get(`[test-player-id="${playerKeys[0]}"]`).get('label').contains('Yes').click();
      cy.get(`[test-player-id="${playerKeys[0]}"]`)
        .find('button')
        .contains('Next')
        .click();
      ...
      cy.log('Intro: instructions and understanding check');
      cy.get(`[test-player-id="${playerKeys[1]}"]`).contains('In this study', { timeout: 5000 });
      ...
      cy.get(`[test-player-id="${playerKeys[1]}"]`).get('label').contains('Yes').click();
      cy.get(`[test-player-id="${playerKeys[1]}"]`)
        .find('button')
        .contains('Next')
        .click();
      ...
      // Video check1
      cy.get(`[test-player-id="${playerKeys[0]}"]`).contains('Check your webcam', { timeout: 5000 });
      // cy.get('[data-test="enableIframe"]').uncheck({force: true}) // default disabled in cypress

      cy.get(`[test-player-id="${playerKeys[0]}"]`).find('input[id="enabled"]').click();
      cy.get(`[test-player-id="${playerKeys[0]}"]`).find('input[id="see"]').click();
      cy.get(`[test-player-id="${playerKeys[0]}"]`).find('input[id="noName"]').click();
      cy.get(`[test-player-id="${playerKeys[0]}"]`)
      ...
      // Video check2
      cy.get(`[test-player-id="${playerKeys[1]}"]`).contains('Check your webcam', { timeout: 5000 });
      // cy.get('[data-test="enableIframe"]').uncheck({force: true}) // default disabled in cypress

      cy.get(`[test-player-id="${playerKeys[1]}"]`).find('input[id="enabled"]').click();
      cy.get(`[test-player-id="${playerKeys[1]}"]`).find('input[id="see"]').click();
      cy.get(`[test-player-id="${playerKeys[1]}"]`).find('input[id="noName"]').click();
      cy.get(`[test-player-id="${playerKeys[1]}"]`)
        .find('input[id="background"]')
        .click();
      ...
      cy.get(`[test-player-id="${playerKeys[1]}"]`).find('[data-test="profile"]', { timeout: 20000 }); // check that profile loaded
      cy.get(`[test-player-id="${playerKeys[1]}"]`).contains('personal opinion');
      cy.get(`[test-player-id="${playerKeys[1]}"]`)
        .get('input[value="Neither agree nor disagree"]')
        .should('not.be.checked'); // check no spillover from p1
      cy.get(`[test-player-id="${playerKeys[1]}"]`).contains('Agree strongly').click();
      cy.get(`[test-player-id="${playerKeys[1]}"]`).find('input[type="submit"]').click();
      cy.get(`[test-player-id="${playerKeys[1]}"]`)
      ...
      cy.get(`[test-player-id="${playerKeys[0]}"]`).contains('as a group', { timeout: 15000 });
      cy.get(`[test-player-id="${playerKeys[0]}"]`)
        .get('input[value="Neither agree nor disagree"]')
      ...
      cy.get(`[test-player-id="${playerKeys[0]}"]`).contains('Quality Feedback Survey', { timeout: 5000 });
      ...
      cy.get(`[test-player-id="${playerKeys[1]}"]`).contains('Quality Feedback Survey', { timeout: 5000 });
      cy.wait(1000); // flake mitigation
      cy.get(`[test-player-id="${playerKeys[1]}"]`)
        .find('[data-responsive-title="Disagree"]')
        .click({
