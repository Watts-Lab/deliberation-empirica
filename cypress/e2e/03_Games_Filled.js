...
describe('All games fill up with extra player in intro steps', () => {
  ...
  it('redirects to sorry on game full', () => {
    ...
    cy.get(`[test-player-id="${playerKeys[1]}"]`).find('label').contains('Yes').click();
    cy.get(`[test-player-id="${playerKeys[1]}"]`).find('button').contains('Next').click();
    ...
    cy.get(`[test-player-id="${playerKeys[1]}"]`).find('input[id="background"]').click();

    // check for alert
    cy.get(`[test-player-id="${playerKeys[1]}"]`).get('button').contains('Next').click();
    cy.get(`[test-player-id="${playerKeys[1]}"]`).contains(
      'Not all of the necessary items were confirmed!',
      { timeout: 5000 },
    );

    cy.get(`[test-player-id="${playerKeys[1]}"]`).find('input[id="safeplace"]').click();
    cy.get(`[test-player-id="${playerKeys[1]}"]`).find('input[id="speakFree"]').click();
    cy.get(`[test-player-id="${playerKeys[1]}"]`)
